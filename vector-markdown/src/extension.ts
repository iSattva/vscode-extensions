...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("vectorMarkdown.openPreview", async (uri?: vscode.Uri) => {
    ...
    vscode.commands.registerCommand("vectorMarkdown.selectTheme", async () => {
      ...
      if (picked === "custom") {
        await vscode.commands.executeCommand("vectorMarkdown.configureCustomTheme");
      }
      PreviewPanel.refreshTheme(themeManager);
    }),

    vscode.commands.registerCommand("vectorMarkdown.configureCustomTheme", async () => {
    ...
    vscode.commands.registerCommand("vectorMarkdown.exportPdf", async (uri?: vscode.Uri) =>
      runExport("PDF", uri, themeManager, logger, (doc) => exportPdf(doc, themeManager))
    ),

    vscode.commands.registerCommand("vectorMarkdown.exportHtml", async (uri?: vscode.Uri) =>
      runExport("HTML", uri, themeManager, logger, (doc) => exportHtml(doc, themeManager))
    ),

    vscode.commands.registerCommand("vectorMarkdown.exportDocx", async (uri?: vscode.Uri) =>
    ...
    vscode.commands.registerCommand("vectorMarkdown.exportAll", async (uri?: vscode.Uri) => {
      const document = await resolveMarkdownDocument(uri);
      if (!document) {
        return;
      }
      await vscode.window.withProgress(
        ...
        async (progress) => {
          ...
          await safeExport("PDF", document, logger, () => exportPdf(document, themeManager));
          progress.report({ message: "HTML" });
          await safeExport("HTML", document, logger, () => exportHtml(document, themeManager));
          progress.report({ message: "DOCX" });
          await safeExport("DOCX", document, logger, async () => (await exportDocx(document, themeManager)).path);
        }
      );
  ...
  context.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration((event) => {
      if (event.affectsConfiguration("vectorMarkdown")) {
        PreviewPanel.refreshTheme(themeManager);
      }
    })
  );
